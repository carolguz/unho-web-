...
const UNHO = {
  ...
  renderNoticias() {
    ...
    if (saved.length === 0) {
      list.innerHTML = '<p style="color:var(--gray);font-size:13px;padding:1rem 0;">Sin noticias publicadas aun. La noticia de UICN aparece por defecto.</p>';
      this.updateStats(); return;
    }
    list.innerHTML = saved.map((n, i) => {
      ...
      return '<div class="li-item" style="gap:14px;align-items:flex-start;">' +
        '<div style="width:70px;height:56px;border-radius:8px;overflow:hidden;background:#f0f0f0;flex-shrink:0;display:flex;align-items:center;justify-content:center;">' +
        (src ? '<img src="' + src + '" style="width:100%;height:100%;object-fit:cover;" onerror="this.parentElement.innerHTML='📷'">' : '<span style="font-size:20px;">📷</span>') +
        '</div>' +
        '<div style="flex:1;min-width:0;"><h4>' + n.titulo + '</h4><p>' + (n.fecha||'') + '</p></div>' +
  ...
  renderGaleria() {
    ...
    list.innerHTML = fotos.map((f, i) => {
      const imgSrc = f.base64
        ? f.base64
        : f.driveId
          ? `https://lh3.googleusercontent.com/d/${f.driveId}`
          : '';
      return `<div class="li-item" style="align-items:flex-start;gap:14px;">
