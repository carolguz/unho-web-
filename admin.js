...
const UNHO = {
  ...
  renderEmpleos() {
    ...
    if (saved.length === 0) {
      list.innerHTML = '<p style="color:var(--gray);font-size:13px;padding:1rem 0;">Sin vacantes publicadas aun.</p>';
      return;
    }
    list.innerHTML = saved.map((e, i) => `
      <div class="li-item">
        <div style="flex:1;"><h4>${e.puesto}</h4><p>${e.tipo || ''} · ${e.ubicacion || ''} · ${e.duracion || ''}</p></div>
  ...
  addEmpleo() {
    ...
    const pdfUrl    = document.getElementById('inp-emp-pdfurl').value.trim();
    const driveId   = document.getElementById('inp-emp-driveid').value.trim();
    if (!puesto) { alert('Ingresa el nombre del puesto'); return; }
    const saved = JSON.parse(localStorage.getItem('unho_empleos') || '[]');
    saved.unshift({ puesto, tipo, duracion, ubicacion, descripcion, requisitos, pdfUrl, driveId, activo: true });
    localStorage.setItem('unho_empleos', JSON.stringify(saved));
    ['inp-emp-puesto','inp-emp-duracion','inp-emp-ubicacion','inp-emp-desc','inp-emp-req','inp-emp-pdfurl','inp-emp-driveid'].forEach(id => { const el=document.getElementById(id); if(el) el.value=''; });
    this.renderEmpleos();
  ...
  renderEquipo() {
    ...
    if (!list) return;
    const saved = JSON.parse(localStorage.getItem('unho_team') || '[]');
    if (saved.length === 0) {
      list.innerHTML = '<p style="color:var(--gray);font-size:13px;padding:1rem 0;">Sin integrantes agregados aun.</p>';
      return;
    }
    list.innerHTML = saved.map((m, i) => {
      const src = m.base64 || (m.driveId ? "https://lh3.googleusercontent.com/d/" + m.driveId : "");
      return `<div class="li-item" style="gap:14px;">
        ${src ? `<img src="${src}" style="width:50px;height:50px;border-radius:50%;object-fit:cover;flex-shrink:0;">` : '<div style="width:50px;height:50px;border-radius:50%;background:#f0f0f0;flex-shrink:0;"></div>'}
        <div style="flex:1;"><h4>${m.nombre}</h4><p>${m.cargo}</p></div>
  ...
  addEquipo() {
    const nombre = document.getElementById('inp-eq-nombre').value.trim();
    const cargo  = document.getElementById('inp-eq-cargo').value.trim();
    const desc   = document.getElementById('inp-eq-desc').value.trim();
    const driveId= document.getElementById('inp-eq-drive').value.trim();
    const base64 = document.getElementById('inp-eq-drive').dataset.base64 || '';
    if (!nombre) { alert('Ingresa el nombre'); return; }
    const saved = JSON.parse(localStorage.getItem('unho_team') || '[]');
    saved.push({ nombre, cargo, descripcion: desc, driveId, base64 });
    localStorage.setItem('unho_team', JSON.stringify(saved));
    ['inp-eq-nombre','inp-eq-cargo','inp-eq-desc'].forEach(id => { const el=document.getElementById(id); if(el) el.value=''; });
    document.getElementById('inp-eq-drive').value = '';
    delete document.getElementById('inp-eq-drive').dataset.base64;
    document.getElementById('eq-foto-preview').style.display = 'none';
    this.renderEquipo();
  ...
  handleEqFileUpload(input) {
    const file = input.files[0]; if (!file) return;
    const reader = new FileReader();
    reader.onload = e => {
      document.getElementById('eq-prev-img').src = e.target.result;
      document.getElementById('eq-foto-preview').style.display = 'flex';
      document.getElementById('inp-eq-drive').dataset.base64 = e.target.result;
      document.getElementById('inp-eq-drive').value = '';
    };
  ...
  previewEqFoto() {
    const id = document.getElementById('inp-eq-drive').value.trim();
    if (!id) { alert('Ingresa el ID de Drive'); return; }
    document.getElementById('eq-prev-img').src = "https://lh3.googleusercontent.com/d/" + id;
    document.getElementById('eq-foto-preview').style.display = 'flex';
  },

  // ── HERO ──
  saveHero() {
    const texto = document.getElementById('inp-hero-texto').value.trim();
    const driveId = document.getElementById('inp-hero-bg-drive').value.trim();
    const base64 = document.getElementById('inp-hero-bg-drive').dataset.base64 || '';
    const heroData = JSON.parse(localStorage.getItem('unho_hero') || '{}');
    if (texto) heroData.texto = texto;
    if (driveId) heroData.bgDriveId = driveId;
    if (base64) heroData.bgBase64 = base64;
    localStorage.setItem('unho_hero', JSON.stringify(heroData));
    this.showMsg('msg-hero');
  },
  ...
  saveHeroTexto() {
    ...
    const heroData = JSON.parse(localStorage.getItem('unho_hero') || '{}');
    heroData.texto = texto;
    localStorage.setItem('unho_hero', JSON.stringify(heroData));
    this.showMsg('msg-hero-texto');
  },
  ...
  handleHeroBg(input) {
    ...
    const reader = new FileReader();
    reader.onload = e => {
      document.getElementById('hero-bg-img').src = e.target.result;
      document.getElementById('hero-bg-preview').style.display = 'block';
      document.getElementById('inp-hero-bg-drive').dataset.base64 = e.target.result;
      document.getElementById('inp-hero-bg-drive').value = '';
    };
    reader.readAsDataURL(file);
  },

  previewHeroBg() {
    const id = document.getElementById('inp-hero-bg-drive').value.trim();
    if (!id) { alert('Ingresa el ID de Drive'); return; }
    document.getElementById('hero-bg-img').src = "https://drive.google.com/thumbnail?id=" + id + "&sz=w800";
    document.getElementById('hero-bg-preview').style.display = 'block';
  },
  ...
  loadHeroData() {
    const heroData = JSON.parse(localStorage.getItem('unho_hero') || '{}');
    if (heroData.texto) {
      const el = document.getElementById('inp-hero-texto');
      if (el) el.value = heroData.texto;
    }
    if (heroData.bgBase64 || heroData.bgDriveId) {
      const src = heroData.bgBase64 || ("https://lh3.googleusercontent.com/d/" + heroData.bgDriveId);
      const img = document.getElementById('hero-bg-img');
      const preview = document.getElementById('hero-bg-preview');
      if (img) { img.src = src; preview.style.display = 'block'; }
    }
  },
  ...
  renderProyectos() {
    ...
    list.innerHTML = items.map((p, i) => {
      const src = p.base64 || (p.driveId ? "https://lh3.googleusercontent.com/d/" + p.driveId : "");
  ...
  previewProyFoto() {
    ...
    if (!id) { alert('Ingresa el ID de Drive'); return; }
    document.getElementById('proy-prev-img').src = "https://lh3.googleusercontent.com/d/" + id;
  ...
  renderNoticias() {
    ...
    if (saved.length === 0) {
      list.innerHTML = '<p style="color:var(--gray);font-size:13px;padding:1rem 0;">Sin noticias publicadas. La noticia de UICN aparece por defecto.</p>';
      this.updateStats(); return;
    }
    list.innerHTML = '';
    saved.forEach((n, i) => {
      const src = n.base64 || n.imageUrl || (n.driveImgId ? 'https://lh3.googleusercontent.com/d/' + n.driveImgId : '');
      const item = document.createElement('div');
      item.className = 'li-item';
      item.style.cssText = 'gap:14px;align-items:flex-start;';
      const imgBox = document.createElement('div');
      imgBox.style.cssText = 'width:70px;height:56px;border-radius:8px;overflow:hidden;background:#f0f0f0;flex-shrink:0;';
      if (src) {
      ...
      info.innerHTML = '<h4>' + n.titulo + '</h4><p>' + (n.fecha||'') + '</p>';
      const actions = document.createElement('div');
      actions.className = 'li-actions';
      const delBtn = document.createElement('button');
      delBtn.className = 'btn-del'; delBtn.textContent = 'Eliminar';
      delBtn.onclick = () => UNHO.deleteNoticia(i);
      actions.appendChild(delBtn);
      item.appendChild(imgBox); item.appendChild(info); item.appendChild(actions);
  ...
  renderParticipa() {
    ...
    list.innerHTML = fotos.map((f, i) => {
      const src = f.base64 || (f.driveId ? `https://lh3.googleusercontent.com/d/${f.driveId}` : '');
  ...
  previewPartFoto() {
    ...
    if (!id) { alert('Ingresa el ID de Drive'); return; }
    document.getElementById('part-prev-img').src = `https://lh3.googleusercontent.com/d/${id}`;
    document.getElementById('part-foto-preview').style.display = 'flex';
  },
  ...
  renderGaleria() {
    ...
    list.innerHTML = '';
    fotos.forEach((f, i) => {
      const imgSrc = f.base64 || (f.driveId ? 'https://lh3.googleusercontent.com/d/' + f.driveId : '');
      const item = document.createElement('div');
      item.className = 'li-item';
      item.style.cssText = 'align-items:flex-start;gap:14px;';
      const imgBox = document.createElement('div');
      imgBox.style.cssText = 'width:80px;height:65px;border-radius:8px;overflow:hidden;background:#f0f0f0;flex-shrink:0;display:flex;align-items:center;justify-content:center;';
      if (imgSrc) {
        const img = document.createElement('img');
        img.src = imgSrc;
      ...
      info.innerHTML = '<h4>' + (f.descripcion || 'Sin descripcion') + '</h4><p>' + (f.driveId ? 'Imagen de Google Drive' : f.base64 ? 'Imagen subida directamente' : 'Sin imagen') + '</p>';
      const actions = document.createElement('div');
      actions.className = 'li-actions';
      const delBtn = document.createElement('button');
      delBtn.className = 'btn-del';
      delBtn.textContent = 'Eliminar';
      delBtn.onclick = () => UNHO.deleteFoto(i);
      actions.appendChild(delBtn);
  ...
  previewGalFoto() {
    ...
    if (!id) { alert('Ingresa el ID de Drive'); return; }
    document.getElementById('gal-prev-img').src = `https://lh3.googleusercontent.com/d/${id}`;
    document.getElementById('gal-foto-preview').style.display = 'flex';
  },
