...
const UNHO = {
  ...
  // ════════════════════════════════  HOJA  ════════════════════════════════
  // Lee una pestaña a traves del puente Apps Script (devuelve filas en JSON)
  async leerTab(tab) {
    try {
  ...
  async renderNoticias() {
    ...
    rows.forEach((n, i) => {
      ...
      item.innerHTML =
        ...
        '<div class="li-actions"><button class="btn-del" onclick="UNHO.deleteNoticia(' + (n._row || (i+2)) + ')">Eliminar</button></div>';
  ...
  async deleteNoticia(row) {
    if (!confirm('Eliminar esta noticia de la hoja?')) return;
    await this.eliminar('Noticias', row);
    setTimeout(() => this.renderNoticias(), 1200);
  },
  ...
  async renderProyectos() {
    ...
    rows.forEach((p, i) => {
      ...
      item.innerHTML =
        ...
        '<div class="li-actions"><button class="btn-del" onclick="UNHO.deleteProyecto(' + (p._row || (i+2)) + ')">Eliminar</button></div>';
      list.appendChild(item);
    });
  },
  ...
  async deleteProyecto(row) {
    if (!confirm('Eliminar este proyecto de la hoja?')) return;
    await this.eliminar('proyectos', row);
    setTimeout(() => this.renderProyectos(), 1200);
  },
  ...
  async renderEmpleos() {
    ...
    list.innerHTML = rows.map((e, i) =>
      '<div class="li-item">' +
        ...
        '<div class="li-actions"><button class="btn-del" onclick="UNHO.deleteEmpleo(' + (e._row || (i+2)) + ')">Eliminar</button></div>' +
      '</div>').join('');
  },
  ...
  async deleteEmpleo(row) {
    if (!confirm('Eliminar esta vacante de la hoja?')) return;
    await this.eliminar('Empleos', row);
    setTimeout(() => this.renderEmpleos(), 1200);
  },
  ...
  async renderEquipo() {
    ...
    rows.forEach((m, i) => {
      ...
      item.innerHTML =
        ...
        '<div class="li-actions"><button class="btn-del" onclick="UNHO.deleteEquipo(' + (m._row || (i+2)) + ')">Eliminar</button></div>';
      list.appendChild(item);
    });
  },
  ...
  async renderGaleria() {
    ...
    rows.forEach((f, i) => {
      ...
      item.innerHTML =
        ...
        '<div class="li-actions"><button class="btn-del" onclick="UNHO.deleteFoto(' + (f._row || (i+2)) + ')">Eliminar</button></div>';
      list.appendChild(item);
    });
  },
  ...
  async deleteFoto(row) {
    if (!confirm('Eliminar esta foto de la hoja?')) return;
    await this.eliminar('Galeria', row);
    setTimeout(() => this.renderGaleria(), 1200);
  },
